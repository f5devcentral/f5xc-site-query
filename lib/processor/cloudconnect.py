    ...
    def run(self) -> dict | None:
        ...
        def process():
            try:
                ...
                if 'cloud_connector' not in self.data[c.SITES_KEY][site].keys():
                    self.data[c.SITES_KEY][site]["cloud_connector"] = dict()
                if cloud_connector_name not in self.data[c.SITES_KEY][site]["cloud_connector"].keys():
                    self.data[c.SITES_KEY][site]["cloud_connector"][cloud_connector_name] = dict()
                self.data[c.SITES_KEY][site]['cloud_connector'][cloud_connector_name]['spec'] = dict()
                self.data[c.SITES_KEY][site]['cloud_connector'][cloud_connector_name]['metadata'] = dict()
                self.data[c.SITES_KEY][site]['cloud_connector'][cloud_connector_name]['system_metadata'] = dict()
                self.data[c.SITES_KEY][site]['cloud_connector'][cloud_connector_name]['spec'] = cloud_connector["data"]["spec"]
                self.data[c.SITES_KEY][site]['cloud_connector'][cloud_connector_name]['metadata'] = cloud_connector["data"]["metadata"]
                self.data[c.SITES_KEY][site]['cloud_connector'][cloud_connector_name]['system_metadata'] = cloud_connector["data"]['system_metadata']
        ...
        if _ccs:
            ...
            if cloud_connectors:
                for cloud_connector in cloud_connectors:
                    ...
                    if site in self.data[c.SITES_KEY]:
